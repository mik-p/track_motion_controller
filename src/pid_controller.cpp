...
double PIDController::pid_factory(double delta_t)
{
  ...
  return _pid_params->kp * sig_error + _pid_params->ki * _pid_params->sum_error +
...
double PIDController::improved_pid_factory(double delta_t)
{
  ...
  double difference =
      -((_pid_params->signal_measured - _pid_params->previous_error) / delta_t);  // get running difference
